...
class MainGUI:
    def __init__(self, master, settings_manager, database_manager):
        ...
        self.category_list = [
            "Alcoholic Beverages", "Baby Foods", "Beverages", "Breads & Bakery", "Breakfast Foods",
            "Candy & Chocolate", "Canned Goods", "Condiments & Spreads", "Dairy, Cheese & Eggs",
            "Deli & Prepared Foods", "Dessert", "Dried Fruits & Nuts", "Food & Beverage Gifts",
            "Fresh Flowers", "Frozen Foods", "Meat & Seafood", "Meat Substitutes", "Pantry Staples",
            "Pastas", "Produce", "Snack Foods", "Arts & Crafts", "Automotive", "Beauty",
            "Clothing & Accessories", "Electronics", "Health", "Home", "Home Improvement", "Office",
            "Outdoors", "Personal Care", "Pets", "Sports", "Toys", "Video Games", "Miscellaneous"]
        self.name_list = []
        ...
        self.mainframe = Frame(
            self.master, height=1200, width=800, bg=self.main_background_color)
        self.mainframe.grid(
            row=0, column=0, rowspan=10, columnspan=12, sticky=W + E + N + S, padx=0, pady=0)
        self.top_button_frame = Frame(
            self.mainframe, bg=self.main_background_color)
        self.top_button_frame.grid(
            row=0, column=0, rowspan=2, columnspan=12, sticky=W + E + N + S, padx=0, pady=0)
        self.settings_button = Button(
            self.top_button_frame, text="Settings",
            font="helvetica", command=self.open_settings_menu)
        self.settings_button.grid(
            row=0, column=0, columnspan=2, sticky=W, padx=(20, 5), pady=(20, 0))
        self.report_button = Button(
            self.top_button_frame, text="Get Report",
            font="helvetica", command=self.open_report_menu)
        self.report_button.grid(
            row=0, column=2, columnspan=2, sticky=W, padx=(10, 10), pady=(20, 0))
        self.item_label = Label(
            self.mainframe, text='Item:',
            bg=self.main_background_color, fg=self.main_foreground_color)
        self.item_label.grid(
            row=1, column=0, sticky=W, padx=(20, 0), pady=(5, 5))
        self.price_label = Label(
            self.mainframe, text='Price:',
            bg=self.main_background_color, fg=self.main_foreground_color)
        self.price_label.grid(
            row=1, column=2, sticky=W, padx=(0, 0), pady=(5, 5))
        self.price_entry = Entry(self.mainframe)
        self.price_entry.grid(
            row=1, column=3, sticky=W, padx=(0, 0), pady=(5, 5))
        self.quantity_label = Label(
            self.mainframe, text='Quantity:',
            bg=self.main_background_color, fg=self.main_foreground_color)
        self.quantity_label.grid(
            row=1, column=4, sticky=W, padx=(0, 0), pady=(5, 5))
        self.quantity_entry = Entry(self.mainframe)
        self.quantity_entry.grid(
            row=1, column=5, sticky=W, padx=(0, 0), pady=(5, 5))
        self.quantity_entry.insert(0, '1')
        self.weight_label = Label(
            self.mainframe, text='Weight (lb):',
            bg=self.main_background_color, fg=self.main_foreground_color)
        self.weight_label.grid(
            row=1, column=6, sticky=W, padx=(0, 0), pady=(5, 5))
        self.weight_entry = Entry(self.mainframe)
        self.weight_entry.grid(
            row=1, column=7, sticky=W, padx=(0, 0), pady=(5, 5))
        self.category_label = Label(
            self.mainframe, text='Category:',
            bg=self.main_background_color, fg=self.main_foreground_color)
        self.category_label.grid(
            row=1, column=8, sticky=W, padx=(0, 0), pady=(5, 5))
        self.category_box = Combobox(
            self.mainframe, textvariable='item_category', state="readonly", width=22)
        self.category_box['values'] = self.category_list
        self.category_box.current(0)
        self.category_box.grid(
            row=1, column=9, columnspan=1, sticky=W, padx=0, pady=(10, 5))
        self.item_submit_button = Button(
            self.mainframe, text="Submit", command=self.submit_item_entry)
        self.item_submit_button.grid(
            row=1, column=10, sticky=W, padx=0, pady=(10, 5))

        self.item_entry = autocomplete_entry_widget.AutocompleteEntry(
            self.name_list, self.mainframe, name_dict=self.name_dict,
            cat_list=self.category_list, cat_box=self.category_box)
        self.item_entry.grid(
            row=1, column=1, sticky=W, padx=(0, 0), pady=(5, 5))

        self.transaction_box = sortable_table.Multicolumn_Listbox(
            self.mainframe, ["Item", "Price", "Quantity", "Category"],
            stripped_rows=("white", "#f2f2f2"), height=16, cell_anchor="center")
        ...
        self.transaction_box.interior.grid(
            row=2, rowspan=8, column=0, columnspan=10, sticky=W + N + S + E, padx=(20, 5), pady=5)

        self.store_label = Label(
            self.mainframe, text='Store:',
            bg=self.main_background_color, fg=self.main_foreground_color)
        self.store_label.grid(
            row=10, column=0, sticky=W, padx=(20, 0), pady=(10, 10))
        self.store_box = Combobox(
            self.mainframe, textvariable='transaction_store', state="readonly")
        ...
        self.store_box.grid(
            row=10, column=1, sticky=W, padx=0, pady=(10, 20))
        self.date_label = Label(
            self.mainframe, text='Date:',
            bg=self.main_background_color, fg=self.main_foreground_color)
        self.date_label.grid(
            row=10, column=2, sticky=W, padx=(10, 0), pady=(10, 20))
        self.date_entry = Entry(self.mainframe)
        self.date_entry.grid(
            row=10, column=3, sticky=W, padx=(0, 0), pady=(10, 20))
        self.date_entry.insert(0, self.today_date)
        self.transaction_submit_button = Button(
            self.mainframe, text="Submit Transaction", command=self.submit_transaction)
        self.transaction_submit_button.grid(
            row=10, column=9, sticky=E, padx=(0, 5), pady=(10, 20))
    ...
    def new_user_alert(self):
        messagebox.showinfo(
            "Hello!", """It looks like this might be your first time here.
             Please start by going into the settings menu and adding a new user and new store.""")
    ...
    def open_settings_menu(self):
        ...
        def add_user_menu():
            ...
            username_label.grid(
                row=0, column=0, sticky=W, padx=(10, 5), pady=(10, 0))
            username_entry = Entry(au_menu)
            username_entry.grid(
                row=0, column=1, sticky=W, padx=(5, 5), pady=(10, 0))
            username_buttons_frame = Frame(au_menu)
            username_buttons_frame.grid(
                row=1, column=0, columnspan=2, sticky=W + E + N + S, padx=0, pady=0)
            add_new_user_button = Button(
                username_buttons_frame, text="Add User", command=add_new_user)
            add_new_user_button.grid(
                row=0, column=0, sticky=W, padx=(10, 5), pady=10)
            close_new_user_button = Button(
                username_buttons_frame, text="Close", command=close_new_user_menu)
            close_new_user_button.grid(
                row=0, column=1, sticky=W, padx=(5, 5), pady=10)
        ...
        def add_store_menu():
            ...
            def add_new_store():
                ...
                if not store_zip:
                    ...
                    return
                self.DBM.push_new_store(
                    store_name, store_address, store_address2, store_city, store_state, store_zip)
                self.DBM.save()
            ...
            store_buttons_frame.grid(
                row=6, column=0, columnspan=2, sticky=W + E + N + S, padx=0, pady=0)
            add_new_store_button = Button(
                store_buttons_frame, text="Add Store", command=add_new_store)
            add_new_store_button.grid(
                row=0, column=0, sticky=W, padx=(10, 5), pady=10)
            close_new_store_button = Button(
                store_buttons_frame, text="Close", command=close_new_store_menu)
            close_new_store_button.grid(
                row=0, column=1, sticky=W, padx=(5, 10), pady=10)
        ...
        background_color_box.grid(
            row=1, column=1, sticky=W, padx=(0, 10), pady=(10, 10))
        settings_button_frame = Frame(settings_menu)
        settings_button_frame.grid(
            row=2, column=0, columnspan=2, sticky=W + E + N + S, padx=0, pady=0)
        save_settings_button = Button(
            settings_button_frame, text="Save", command=save_settings)
        save_settings_button.grid(
            row=0, column=0, sticky=W, padx=(10, 5), pady=(5, 10))
        add_user_button = Button(
            settings_button_frame, text="Add User", command=add_user_menu)
        add_user_button.grid(
            row=0, column=1, sticky=W, padx=(5, 5), pady=(5, 10))
        add_store_button = Button(
            settings_button_frame, text="Add Store", command=add_store_menu)
        add_store_button.grid(
            row=0, column=2, sticky=W, padx=(5, 5), pady=(5, 10))
        close_settings_button = Button(
            settings_button_frame, text="Close", command=close_settings_menu)
        close_settings_button.grid(
            row=0, column=3, sticky=W, padx=(5, 10), pady=(5, 10))
    ...
    def open_report_menu(self):
        ...
        def open_report_result_menu(report_data, query_summary, item_search):
            """report_data_indexes: 0:username_name, 1:product_name, 2:transaction_price,
             3:product_lowest_price, 4:product_average_price,
             5:product_highest_price, 6:product_times_purchased,
             7:product_category, 8:product_weight_based_price, 9:store_name,
             10:transaction_quantity, 11:transaction_date, 12:transaction_weight"""
            ...
            report_title_label = Label(
                report_result_menu, text='Here is your Report:', font="Helvetica")
            report_title_label.grid(
                row=0, column=0, sticky=W, padx=(20, 5), pady=(10, 5))

            if not query_summary:
                column_names = [
                    "Username", "Store", "Item", "Price", "Quantity", "Category", "Date"]
                report_box = sortable_table.Multicolumn_Listbox(
                    report_result_menu, column_names, stripped_rows=("white", "#f2f2f2"),
                    height=16, cell_anchor="center")
                ...
                report_box.interior.grid(
                    row=1, rowspan=8, column=0, columnspan=10,
                    sticky=W + N + S + E, padx=20, pady=10)
                ...
                for i in report_data:
                    ...
                    report_box.insert_row(
                        [i[0], i[9], i[1], '$' + str(i[2]) + '/lb' if i[8] else '$' + str(i[2]),
                         i[10], i[7], i[11]])

                category_overview_label = Label(
                    report_result_menu, text='Category Overview:', font="Helvetica")
                category_overview_label.grid(
                    row=10, column=0, sticky=W, padx=(20, 0), pady=3)
                category_label_dict = {}
                for category, total in category_dict.items():
                    if total != 0:
                        category_label_dict.setdefault(
                            'category_label' + str(used_rows), Label(
                                report_result_menu,
                                text=category + ": $" + str(format(total, '.2f'))))
                        category_label_dict[
                            'category_label' + str(used_rows)].grid(
                            row=int((used_rows / 4) + 10), column=used_rows % 4,
                            sticky=W, padx=(20, 0), pady=3)
                        used_rows += 1

                store_overview_label = Label(
                    report_result_menu, text='Store Overview:', font="Helvetica")
                store_overview_label.grid(
                    row=int((used_rows / 4) + 11), column=0, sticky=W, padx=(20, 0), pady=3)
                store_label_dict = {}
                store_column = 1
                for store, total in store_dict.items():
                    if total != 0:
                        store_label_dict.setdefault(
                            'category_label' + str(used_rows), Label(
                                report_result_menu, text=store + ": $" + str(format(total, '.2f'))))
                        store_label_dict[
                            'category_label' + str(used_rows)].grid(
                            row=int((used_rows / 4) + 11), column=store_column,
                            sticky=W, padx=(20, 0), pady=3)
                        used_rows += 1
                        if store_column >= 3:
                            store_column = 0
                ...
                total_overview_label = Label(
                    report_result_menu, text='Total Spending: $' + str(
                        format(item_total, '.2f')), font="Helvetica")
                total_overview_label.grid(
                    row=int((used_rows / 4) + 12), column=0, sticky=W, padx=(20, 0), pady=3)

                if item_search:
                    item_highest_label = Label(
                        report_result_menu,
                        text='Item Highest Price: $' + str(report_data[0][5]) + '/lb'
                        if report_data[0][8] else 'Item Highest Price: $' + str(report_data[0][5]),
                        font="Helvetica")
                    item_highest_label.grid(
                        row=int((used_rows / 4) + 13), column=0, sticky=W, padx=(20, 0), pady=3)
                    item_average_label = Label(
                        report_result_menu,
                        text='Item Average Price: $' + str(report_data[0][4]) + '/lb'
                        if report_data[0][8] else 'Item Average Price: $' + str(report_data[0][4]),
                        font="Helvetica")
                    item_average_label.grid(
                        row=int((used_rows / 4) + 14), column=0, sticky=W, padx=(20, 0), pady=3)
                    item_lowest_label = Label(
                        report_result_menu,
                        text='Item Lowest Price: $' + str(report_data[0][3]) + '/lb'
                        if report_data[0][8] else 'Item Lowest Price: $' + str(report_data[0][3]),
                        font="Helvetica")
                    item_lowest_label.grid(
                        row=int((used_rows / 4) + 15), column=0, sticky=W, padx=(20, 0), pady=3)
                    close_settings_button = Button(
                        report_result_menu,
                        text="Close", command=lambda: report_result_menu.destroy())
                    close_settings_button.grid(
                        row=int((used_rows / 4) + 16), column=0,
                        sticky=W, padx=(20, 0), pady=(5, 10))

                else:
                    close_settings_button = Button(
                        report_result_menu, text="Close",
                        command=lambda: report_result_menu.destroy())
                    close_settings_button.grid(
                        row=int((used_rows / 4) + 13), column=0,
                        sticky=W, padx=(20, 0), pady=(5, 10))
            else:
                ...
                report_box.interior.column(3, width=100)
                report_box.interior.grid(
                    row=1, rowspan=8, column=0, columnspan=4,
                    sticky=W + N + S + E, padx=20, pady=10)
                ...
                for category, total in store_dict.items():
                    if total != 0:
                        store_label_dict.setdefault(
                            'store_label' + str(used_rows), Label(
                                report_result_menu,
                                text=category + ": $" + str(format(total, '.2f'))))
                        store_label_dict[
                            'store_label' + str(used_rows)].grid(
                            row=int((used_rows / 3) + 10), column=used_rows % 3,
                            sticky=W, padx=(20, 0), pady=3)
                        used_rows += 1

                total_overview_label = Label(
                    report_result_menu, text='Total Spending: $' + str(format(item_total, '.2f')),
                    font="Helvetica")
                total_overview_label.grid(
                    row=int((used_rows / 3) + 11), column=0, sticky=W, padx=(20, 0), pady=3)

                close_settings_button = Button(
                    report_result_menu, text="Close", command=lambda: report_result_menu.destroy())
                close_settings_button.grid(
                    row=int((used_rows / 3) + 12), column=0, sticky=W, padx=(20, 0), pady=(5, 10))
        ...
        def submit_query():
            ...
            open_report_result_menu(
                self.DBM.retrieve_report_query(
                    query_user, query_store, query_item, query_category, query_date1, query_date2,
                    query_summary),
                query_summary, item_search)
        ...
        report_username_box = Combobox(
            report_menu, textvariable='username', state="readonly", width=22)
        ...
        report_store_box = Combobox(
            report_menu, textvariable='storename', state="readonly", width=22)
        ...
        report_item_entry = autocomplete_entry_widget.AutocompleteEntry(
            self.name_list, report_menu, width=25)
        ...
        report_category_box = Combobox(
            report_menu, textvariable='report_item_category', state="readonly", width=22)
        ...
        summary_report_check = Checkbutton(
            report_check_frame, text="Transaction Summary Report", variable=summary_report_var)
        summary_report_check.grid(
            row=3, column=0, sticky=W, padx=(10, 5), pady=5)
        report_instruction_frame = Frame(report_menu)
        report_instruction_frame.grid(
            row=4, column=0, columnspan=4, sticky=W + E, padx=0, pady=(3, 0))
        report_instruction_label = Label(
            report_instruction_frame, text='Instructions:', font=("Helvetica", 18))
        report_instruction_label.grid(
            row=0, column=0, sticky=W, padx=(10, 5), pady=0)
        report_instruction_label2 = Label(
            report_instruction_frame, text='• Form a query using the fields above.',
            font=("Helvetica", 11))
        report_instruction_label2.grid(
            row=1, column=0, sticky=W, padx=(18, 5), pady=5)
        report_instruction_label3 = Label(
            report_instruction_frame, text='• Leave unnecessary fields blank.',
            font=("Helvetica", 11))
        report_instruction_label3.grid(
            row=2, column=0, sticky=W, padx=(18, 5), pady=5)
        report_instruction_label3 = Label(
            report_instruction_frame, text='• Use date format YYYY-MM-DD.',
            font=("Helvetica", 11))
        report_instruction_label3.grid(
            row=3, column=0, sticky=W, padx=(18, 5), pady=5)
        report_instruction_label4 = Label(
            report_instruction_frame, text='• You can enter only one date.',
            font=("Helvetica", 11))
        report_instruction_label4.grid(
            row=4, column=0, sticky=W, padx=(18, 5), pady=5)
        report_instruction_label5 = Label(
            report_instruction_frame,
            text='• Transaction Summary Reports only show transaction totals.',
            font=("Helvetica", 11))
        report_instruction_label5.grid(
            row=5, column=0, sticky=W, padx=(18, 5), pady=5)
        report_button_frame = Frame(report_menu)
        report_button_frame.grid(
            row=5, column=0, columnspan=4, sticky=W + E, padx=0, pady=(3, 0))
        submit_query_button = Button(
            report_button_frame, text="Get Report", command=submit_query)
        submit_query_button.grid(
            row=0, column=0, sticky=W, padx=(10, 5), pady=(5, 10))
        close_report_button = Button(
            report_button_frame, text="Close", command=close_report_menu)
        close_report_button.grid(
            row=0, column=1, sticky=W, padx=(5, 10), pady=(5, 10))
        ...
        if item_name in self.name_dict:
            if not item_weight or item_weight == '0':
                if self.name_dict[item_name][1]:
                    self.error_alert(
                        'Please set a weight for this item as you have done previously.')
                    return
        self.transaction_box.clear()
        self.transaction_dict.setdefault(
            item_name, [item_price, item_quantity, item_category, weight_based_price, item_weight])
    ...
    def submit_transaction(self):
        ...
        if store == "No Stores Added":
            self.error_alert(
                "Please add a store in the settings menu before submitting a transaction.")
            return
        ...
        if self.active_user == "Unidentified User":
            self.error_alert(
                "Please add a new user in the settings menu before submitting a transaction.")
            return
        transaction_list = []
        transaction_total = 0
        ...
        for i in transaction_list:
            ...
            transaction_total += (
                    (float(i[1]) * float(i[5])) * int(i[2])) if i[4] else (float(i[1]) * int(i[2]))
        ...
        for i in self.name_dict:
            self.name_list.append(i)
        self.item_entry.name_dict = self.name_dict
