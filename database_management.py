...
class DatabaseManager:
    ...
    product_weight_based_price BOOLEAN NOT NULL
            ...
            self.cursor.execute("""CREATE TABLE username (
    ...
    username_name STRING  UNIQUE
            ...
            self.cursor.execute("""CREATE TABLE store_transaction (
    ...
    transaction_date     DATE    NOT NULL
            ...
            self.cursor.execute("""CREATE TABLE transaction_summary (
    ...
    def push_transaction(self, product_detail_list, username, store, date):
        ...
        self.cursor.execute('''INSERT INTO store_transaction
                 (username_id, store_id, product_id, transaction_price, transaction_weight, transaction_quantity, transaction_date) VALUES ((SELECT username_id FROM username WHERE username_name = ?),
                  (SELECT store_id FROM store WHERE store_name = ? AND store_city = ?), (SELECT product_id FROM product WHERE product_name = ?),?,?,?,?)''',
                            (username, store_name, store_city, product_name, product_price, product_weight, product_quantity, date))
    ...
    def retrieve_report_query(self, user, store, item, category, date1, date2, query_summary):
        ...
        if not query_summary:
            query_text = ["""SELECT username_name, product_name, transaction_price, product_lowest_price, product_average_price, product_highest_price, product_times_purchased, product_category, product_weight_based_price, 
            ...
            query_text = ["""SELECT username_name, store_name, summary_date, summary_total
                       FROM transaction_summary INNER JOIN username on username.username_id = transaction_summary.username_id
